...
class WarmupCosineSchedule(_LRScheduler):
    """带非线性预热和周期性重启的余弦退火调度器"""

    def __init__(self, optimizer, warmup_epochs, total_epochs,
                 min_lr=1e-6, restart_cycle=30, last_epoch=-1):
        """
        Args:
            warmup_epochs (int): 预热阶段epoch数
            total_epochs (int): 总训练epoch数（用于进度跟踪）
            min_lr (float): 最小学习率边界值
            restart_cycle (int): 余弦退火重启周期（epoch数）
        """
    ...
    def get_lr(self):
        """动态计算各参数组的学习率"""
        if self.last_epoch < self.warmup_epochs:  # 预热阶段
